...
def test_heat_rock_extends_sun(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("torkoal", ["sunnyday", "tackle", "tackle", "tackle"], item="heatrock")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=42,
    )
...
@pytest.mark.xfail(strict=False, reason="switch-in abilities at battle start not yet wired")
def test_drought_ability_sets_sun_on_entry(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("torkoal", ["tackle", "tackle", "tackle", "tackle"], ability="drought")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
...
def test_sun_boosts_fire_damage(fresh_battle, step_turn, hp_of):
    ...
    assert sun_dmg > base_dmg, f"sun should boost fire dmg (base={base_dmg}, sun={sun_dmg})"
...
@pytest.mark.xfail(strict=False, reason="solar beam charge skip in sun not yet verified")
...
def test_meteor_beam_boosts_spa_on_charge_turn(fresh_battle, step_turn, hp_of, boost_of):
    state, prng = fresh_battle(
...
@pytest.mark.xfail(strict=False, reason="weather-scaled recovery moves not yet implemented")
...
@pytest.mark.xfail(strict=False, reason="chlorophyll x2 speed in sun verified in speed file")
def test_chlorophyll_doubles_speed_in_sun(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("venusaur", ["energyball", "tackle", "tackle", "tackle"], ability="chlorophyll")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
    state.battle_state[OFF_FIELD + F_WEATHER] = WEATHER_SUN
...
def test_sand_stream_sets_sand_on_entry(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("tyranitar", ["tackle", "tackle", "tackle", "tackle"], ability="sandstream")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
...
def test_snow_warning_sets_snow_on_entry(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("abomasnow", ["tackle", "tackle", "tackle", "tackle"], ability="snowwarning")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
...
def test_slush_rush_doubles_speed_in_snow(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("sandslash", ["icicleSpear" if False else "tackle",
                                "tackle", "tackle", "tackle"], ability="slushrush")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
...
@pytest.mark.xfail(strict=False, reason="misty terrain dragon halving not yet validated")
...
@pytest.mark.xfail(strict=False, reason="utility umbrella weather immunity not yet wired")
def test_utility_umbrella_blocks_sand_chip(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["splash", "tackle", "tackle", "tackle"], item="utilityumbrella")],
        [MonSpec("snorlax", ["splash", "tackle", "tackle", "tackle"])],
        seed=42,
    )
    state.battle_state[OFF_FIELD + F_WEATHER] = WEATHER_SAND
...
@pytest.mark.xfail(strict=False, reason="air lock / cloud nine suppression not yet wired")
def test_air_lock_suppresses_sand_chip(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["splash", "tackle", "tackle", "tackle"], ability="airlock")],
        [MonSpec("snorlax", ["splash", "tackle", "tackle", "tackle"])],
        seed=42,
    )
    state.battle_state[OFF_FIELD + F_WEATHER] = WEATHER_SAND
...
def test_drizzle_sets_rain_on_entry(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("politoed", ["tackle", "tackle", "tackle", "tackle"], ability="drizzle")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
