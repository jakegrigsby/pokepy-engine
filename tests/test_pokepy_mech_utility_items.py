...
from pokepy.core.constants import (
    ...
    STATUS_BURN, STATUS_TOXIC,
    ITEM_TOXIC_ORB, ITEM_FLAME_ORB, ITEM_WHITE_HERB, ITEM_AIR_BALLOON,
    ITEM_EJECT_BUTTON, ITEM_RED_CARD, ITEM_BOOSTER_ENERGY,
)
...
def test_white_herb_clears_negatives(fresh_battle, step_turn, boost_of):
    ...
    state, prng = fresh_battle(
        [MonSpec("latios", ["dracometeor", "tackle", "tackle", "tackle"], item="whiteherb")],
...
def test_eject_button_triggers_switch(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["splash"]*4, item="ejectbutton"), MonSpec("blissey", ["splash"]*4)],
        [MonSpec("garchomp", ["dragonclaw", "tackle", "tackle", "tackle"])],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
...
def test_red_card_forces_attacker_switch(fresh_battle, step_turn):
    state, prng = fresh_battle(
        ...
        [MonSpec("garchomp", ["dragonclaw", "tackle", "tackle", "tackle"]), MonSpec("blissey", ["splash"]*4)],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
...
@pytest.mark.xfail(strict=False, reason="booster energy activates protosynthesis without sun")
...
def test_power_herb_skips_solar_beam_charge(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("venusaur", ["solarbeam", "tackle", "tackle", "tackle"], item="powerherb")],
        [MonSpec("snorlax", ["splash"]*4)],
        seed=1,
    )
