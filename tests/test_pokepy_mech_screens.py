...
from pokepy.core.constants import (
    OFF_FIELD, F_SCREENS_0, F_SCREENS_1, F_WEATHER, OFF_META, M_WEATHER_TURNS,
    SCREEN_REFLECT_SHIFT, SCREEN_LIGHTSCREEN_SHIFT, SCREEN_AURORAVEIL_SHIFT,
    SCREEN_TAILWIND_SHIFT, SCREEN_SAFEGUARD_SHIFT, SCREEN_MIST_SHIFT,
    SCREEN_MASK_2BIT, SCREEN_MASK_3BIT,
    WEATHER_SNOW,
)

def _screen_turns(state, side: int, shift: int, mask=SCREEN_MASK_3BIT) -> int:
    word = int(state.battle_state[OFF_FIELD + (F_SCREENS_0 if side == 0 else F_SCREENS_1)]) & 0xFFFF
...
def test_light_clay_extends_reflect(fresh_battle, step_turn):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["reflect", "tackle", "tackle", "tackle"], item="lightclay")],
        [MonSpec("garchomp", ["tackle"]*4)],
        seed=1,
    )
    step_turn(state, prng, 0, 0)
