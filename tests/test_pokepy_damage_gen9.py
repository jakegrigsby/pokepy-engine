...
from pokepy.core.constants import (
    OFF_SIDE0, OFF_SIDE1, OFF_META, POKEMON_SIZE, M_ACTIVE0, STATE_SIZE,
    TYPE_NORMAL, TYPE_GROUND, TYPE_DRAGON, TYPE_FLYING, TYPE_GRASS,
    ABILITY_LEVITATE, ABILITY_MOLD_BREAKER,
)
...
def _make_battle(
    atk_species=1, def_species=1,
    atk_type1=TYPE_NORMAL, atk_type2=TYPE_NORMAL,
    def_type1=TYPE_NORMAL, def_type2=TYPE_NORMAL,
    atk_ability=0, def_ability=0,
    level=100, atk_atk=200, atk_spa=200,
    def_def=200, def_spd=200, hp=300,
):
...
def test_smoke_runs(data):
    ...
    dmg = calc_damage_gen9(
        battle, atk_side=0, move_idx=0,
        player_moves=moves, opp_moves=moves,
        game_data=gd, move_effects=me, type_chart=gd.type_chart,
...
def test_status_move_zero_damage(data):
    ...
    moves[0, 0] = growl
    dmg = calc_damage_gen9(
        battle, 0, 0, moves, moves, gd, me, gd.type_chart,
        gen5_prng=Gen5PRNG((1, 2, 3, 4)),
    )
    assert dmg == 0
...
def test_levitate_immune_to_ground(data):
    ...
    battle = _make_battle(def_ability=ABILITY_LEVITATE, def_type1=TYPE_GRASS, def_type2=TYPE_GRASS)
    moves = np.zeros((6, 4), dtype=np.int16)
    moves[0, 0] = eq
    dmg = calc_damage_gen9(battle, 0, 0, moves, moves, gd, me, gd.type_chart,
                            gen5_prng=Gen5PRNG((1, 2, 3, 4)))
    assert dmg == 0
...
def test_mold_breaker_bypasses_levitate(data):
    ...
    battle = _make_battle(
        ...
        def_type1=TYPE_GRASS, def_type2=TYPE_GRASS,
    )
    moves = np.zeros((6, 4), dtype=np.int16)
    moves[0, 0] = eq
    dmg = calc_damage_gen9(battle, 0, 0, moves, moves, gd, me, gd.type_chart,
...
def test_super_effective_more_than_neutral(data):
    ...
    seed = (1, 2, 3, 4)
    d1 = calc_damage_gen9(b1, 0, 0, moves, moves, gd, me, gd.type_chart, gen5_prng=Gen5PRNG(seed))
    d2 = calc_damage_gen9(b2, 0, 0, moves, moves, gd, me, gd.type_chart, gen5_prng=Gen5PRNG(seed))
    assert d2 > d1
