...
from pokepy.core.constants import (
    ...
    M_ACTIVE0, M_ACTIVE1, M_WEATHER_TURNS, F_WEATHER, F_LAST_MOVE_0, F_LAST_MOVE_1,
    WEATHER_SUN,
)
...
def _run_pair(fresh_battle, step_turn, hp_of, attacker, target, move, ability_a,
              ability_b=NO_OP, seed: int = 7, side_to_assert: int = 1):
    ...
    return (pre_a - hp_of(state_a, side_to_assert),
            pre_b - hp_of(state_b, side_to_assert))
...
def test_huge_power_doubles_physical_damage(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "azumarill", "snorlax", "aquajet", "hugepower",
                               seed=11)
...
def test_pure_power_doubles_physical_damage(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "medicham", "snorlax", "machpunch", "purepower",
                               seed=12)
    assert boosted > plain
    assert boosted >= int(plain * 1.7)
...
def test_adaptability_boosts_stab(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "dragapult", "snorlax", "dragonpulse", "adaptability",
                               seed=13)
    # 2.0 / 1.5 = 1.33x
    assert boosted > plain
...
def test_sheer_force_boosts_secondary_effect_move(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "tyranitar", "snorlax", "ironhead", "sheerforce",
                               seed=14)
    assert boosted > plain
    assert boosted >= int(plain * 1.2)
...
def test_tough_claws_boosts_contact_move(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "scizor", "snorlax", "closecombat", "toughclaws",
                               seed=15)
    assert boosted > plain
...
def test_tough_claws_does_not_boost_non_contact(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "scizor", "snorlax", "rockslide", "toughclaws",
                               seed=16)
...
def test_iron_fist_boosts_punch_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "blaziken", "snorlax", "firepunch", "ironfist",
                               seed=17)
    assert boosted > plain
...
def test_strong_jaw_boosts_bite_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "tyrantrum", "snorlax", "crunch", "strongjaw",
                               seed=18)
    assert boosted > plain
...
def test_mega_launcher_boosts_pulse_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "blastoise", "snorlax", "aurasphere", "megalauncher",
                               seed=19)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_punk_rock_boosts_sound_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "toxtricity", "snorlax", "boomburst", "punkrock",
                               seed=20)
    assert boosted > plain
    assert boosted >= int(plain * 1.2)
...
def test_reckless_boosts_recoil_move(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "staraptor", "snorlax", "doubleedge", "reckless",
                               seed=21)
    assert boosted > plain
    assert boosted >= int(plain * 1.1)
...
def test_steelworker_boosts_steel_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "duraludon", "snorlax", "flashcannon", "steelworker",
                               seed=22)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_dragons_maw_boosts_dragon_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "dragapult", "snorlax", "dragonpulse", "dragonsmaw",
                               seed=23)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_rocky_payload_boosts_rock_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "garganacl", "snorlax", "stoneedge", "rockypayload",
                               seed=1)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
@pytest.mark.xfail(strict=False, reason="stakeout requires synthetic just-switched flag and engine support")
def test_stakeout_doubles_vs_just_switched(fresh_battle, step_turn, hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("grafaiai", ["tackle", "tackle", "tackle", "tackle"], ability="stakeout")],
...
def test_tinted_lens_doubles_resisted_damage(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "venomoth", "heracross", "xscissor", "tintedlens",
                               seed=26)
    assert boosted > plain
    assert boosted >= int(plain * 1.7)
...
def test_sniper_boosts_crit_damage(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "frosmoth", "snorlax", "frostbreath", "sniper",
                               seed=27)
    assert boosted > plain
...
def test_water_bubble_doubles_water_damage(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "araquanid", "snorlax", "liquidation", "waterbubble",
                               seed=28)
    assert boosted > plain
    assert boosted >= int(plain * 1.7)
...
def test_neuroforce_boosts_supereffective(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "garchomp", "tyranitar", "earthquake", "neuroforce",
                               seed=29)
    assert boosted > plain
...
def test_analytic_boosts_when_moving_last(fresh_battle, step_turn, hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("snorlax", ["bodyslam", "tackle", "tackle", "tackle"], ability="analytic")],
...
def test_gorilla_tactics_boosts_physical(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "darmanitan", "snorlax", "earthquake", "gorillatactics",
                               seed=31)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_hustle_boosts_physical(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "dragonite", "snorlax", "earthquake", "hustle",
                               seed=32)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_overgrow_boosts_grass_when_low_hp(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("venusaur", ["energyball", "tackle", "tackle", "tackle"], ability="overgrow")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=33,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("venusaur", ["energyball", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=33,
    )
...
def test_blaze_boosts_fire_when_low_hp(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("blaziken", ["flamethrower", "tackle", "tackle", "tackle"], ability="blaze")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=34,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("blaziken", ["flamethrower", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=34,
    )
    _set_low_hp(state_a, 0)
...
def test_torrent_boosts_water_when_low_hp(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("swampert", ["surf", "tackle", "tackle", "tackle"], ability="torrent")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=35,
    )
    state_b, prng_b = fresh_battle(
...
def test_swarm_boosts_bug_when_low_hp(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("heracross", ["xscissor", "tackle", "tackle", "tackle"], ability="swarm")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=36,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("heracross", ["xscissor", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=36,
    )
    _set_low_hp(state_a, 0)
...
def test_solar_power_boosts_special_in_sun(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("venusaur", ["energyball", "tackle", "tackle", "tackle"], ability="solarpower")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=37,
    )
    state_b, prng_b = fresh_battle(
...
@pytest.mark.xfail(strict=False, reason="EOT chip from Solar Power not yet wired in pokepy")
def test_solar_power_chip_in_sun(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("venusaur", ["tackle", "tackle", "tackle", "tackle"], ability="solarpower")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], ability=NO_OP)],
        seed=38,
    )
...
def test_mold_breaker_ignores_levitate(fresh_battle, step_turn, hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("excadrill", ["earthquake", "tackle", "tackle", "tackle"], ability="moldbreaker")],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=39,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("excadrill", ["earthquake", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=39,
    )
...
def test_teravolt_ignores_defender_ability(fresh_battle, step_turn, hp_of):
    ...
    from pokepy.core.constants import OFF_SIDE1
    state_a, prng_a = fresh_battle(
        [MonSpec("zekrom", ["earthquake", "tackle", "tackle", "tackle"], ability="teravolt")],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=40,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("zekrom", ["earthquake", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=40,
    )
    pre_a = int(state_a.battle_state[OFF_SIDE1 + 1])
...
def test_turboblaze_ignores_defender_ability(fresh_battle, step_turn, hp_of):
    ...
    from pokepy.core.constants import OFF_SIDE1
    state_a, prng_a = fresh_battle(
        [MonSpec("reshiram", ["earthquake", "tackle", "tackle", "tackle"], ability="turboblaze")],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=41,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("reshiram", ["earthquake", "tackle", "tackle", "tackle"], ability=NO_OP)],
        [MonSpec("blissey", ["tackle", "tackle", "tackle", "tackle"], ability="levitate")],
        seed=41,
    )
    pre_a = int(state_a.battle_state[OFF_SIDE1 + 1])
...
def test_sharpness_boosts_slicing_move(fresh_battle, step_turn, hp_of):
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "ironvaliant", "snorlax", "psychocut", "sharpness",
                               seed=42)
    assert boosted > plain
    assert boosted >= int(plain * 1.35)
...
def test_skill_link_maximizes_multihit_damage(fresh_battle, step_turn, hp_of):
    ...
    boosted, plain = _run_pair(fresh_battle, step_turn, hp_of,
                               "cloyster", "snorlax", "iciclespear", "skilllink",
                               seed=43)
    assert boosted >= plain
