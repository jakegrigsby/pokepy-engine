...
@pytest.mark.xfail(
    strict=False, reason="STAB calc with tera matching original type = 2.0x"
)
...
def test_tera_blast_changes_to_tera_type(fresh_battle, step_turn, hp_of):
    ...
    state, prng = fresh_battle(
        [
            MonSpec(
                "garchomp",
                ["terablast", "tackle", "tackle", "tackle"],
                tera_type=TYPE_FIGHTING,
            )
        ],
        [MonSpec("snorlax", ["tackle"] * 4)],
        seed=1,
