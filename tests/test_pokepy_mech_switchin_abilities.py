...
from pokepy.core.constants import (
    ...
    M_ACTIVE0, M_ACTIVE1,
    F_WEATHER, F_TERRAIN,
    M_WEATHER_TURNS, M_TERRAIN_TURNS,
    ...
    ABILITY_ELECTRIC_SURGE, ABILITY_GRASSY_SURGE, ABILITY_PSYCHIC_SURGE,
    ABILITY_MISTY_SURGE,
    ABILITY_INTIMIDATE, ABILITY_TRACE, ABILITY_DOWNLOAD,
    ABILITY_DEFIANT, ABILITY_COMPETITIVE,
    ABILITY_CLEAR_BODY, ABILITY_INNER_FOCUS, ABILITY_OBLIVIOUS,
    ABILITY_INTREPID_SWORD, ABILITY_DAUNTLESS_SHIELD,
    ABILITY_ORICHALCUM_PULSE, ABILITY_HADRON_ENGINE,
...
@pytest.mark.xfail(strict=False, reason="hyper cutter is atk-specific block; pokepy may treat it as full Clear Body")
...
@pytest.mark.xfail(strict=False, reason="Imposter transform not implemented in apply_switch_in_ability")
...
def test_quark_drive_booster_energy_not_active_before_switchin(fresh_battle):
    ...
    state, _ = fresh_battle(
        [MonSpec("snorlax", ["tackle"]*4),
         MonSpec("ironvaliant", ["tackle"]*4, ability="quarkdrive", item="boosterenergy")],
        [MonSpec("garchomp", ["tackle"]*4)],
        seed=3201,
    )
    bench_off = OFF_SIDE0 + POKEMON_SIZE
    assert get_effective_speed(state.battle_state, bench_off) == int(state.battle_state[bench_off + 11])
...
def test_protosynthesis_booster_energy_consumed_off_field(fresh_battle, step_turn):
    ...
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["tackle"]*4),
         MonSpec("greattusk", ["tackle"]*4, ability="protosynthesis", item="boosterenergy")],
        [MonSpec("garchomp", ["tackle"]*4)],
        seed=33,
    )
    step_turn(state, prng, 5, 0)
...
@pytest.mark.xfail(strict=False, reason="Neutralizing Gas is not implemented in apply_switch_in_ability")
...
@pytest.mark.xfail(strict=False, reason="Air Lock weather suppression not handled in apply_switch_in_ability")
...
@pytest.mark.xfail(strict=False, reason="Cloud Nine weather suppression not handled in apply_switch_in_ability")
