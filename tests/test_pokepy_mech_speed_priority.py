...
from pokepy.core.constants import (
    OFF_FIELD, OFF_META, F_TRICK_ROOM, M_WEATHER_TURNS, F_WEATHER, WEATHER_RAIN, WEATHER_SUN,
)
...
def test_swift_swim_doubles_in_rain(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("kingdra", ["hydropump", "tackle", "tackle", "tackle"], ability="swiftswim")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
...
def test_chlorophyll_doubles_in_sun(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("venusaur", ["gigadrain", "tackle", "tackle", "tackle"], ability="chlorophyll")],
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        seed=1,
    )
...
@pytest.mark.xfail(strict=False, reason="sucker punch requires target to be using attack")
