...
from pokepy.core.constants import (
    ...
    M_ACTIVE0, M_ACTIVE1, F_HAZARDS_0, F_HAZARDS_1,
    HAZARD_STEALTH_ROCK, HAZARD_SPIKES,
    ITEM_LIFE_ORB, ITEM_LEFTOVERS, ITEM_HEAVY_DUTY_BOOTS, ITEM_FOCUS_SASH,
    ITEM_AIR_BALLOON, ITEM_WEAKNESS_POLICY, ITEM_ROCKY_HELMET, ITEM_LOADED_DICE,
    ITEM_BLACK_SLUDGE, ITEM_SITRUS_BERRY,
)
...
def test_choice_band_boosts_physical_damage(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"], item="choiceband")],
...
def test_choice_specs_boosts_special_damage(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("alakazam", ["psychic", "tackle", "tackle", "tackle"], item="choicespecs")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"])],
        seed=2,
    )
    state_b, prng_b = fresh_battle(
    ...
    plain_dmg = hp_pre - hp_of(state_b, 1)
    assert specs_dmg > plain_dmg, f"choice specs should boost: {specs_dmg} vs {plain_dmg}"
...
def test_life_orb_boosts_damage(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"], item="lifeorb")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"])],
        seed=4,
    )
    state_b, prng_b = fresh_battle(
...
def test_expert_belt_only_supereffective(fresh_battle, step_turn, hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"], item="expertbelt")],
...
def test_expert_belt_no_boost_neutral(fresh_battle, step_turn, hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("garchomp", ["tackle", "tackle", "tackle", "tackle"], item="expertbelt")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"])],
        seed=7,
    )
    state_b, prng_b = fresh_battle(
...
def test_assault_vest_reduces_special_damage(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        ...
        [MonSpec("tyranitar", ["tackle", "tackle", "tackle", "tackle"], item="assaultvest")],
        seed=9,
    )
    state_b, prng_b = fresh_battle(
        [MonSpec("greninja", ["icebeam", "tackle", "tackle", "tackle"])],
...
def test_heavy_duty_boots_ignore_stealth_rock(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("garchomp", ["splash", "tackle", "tackle", "tackle"]),
         MonSpec("skarmory", ["splash", "tackle", "tackle", "tackle"], item="heavydutyboots")],
...
def test_air_balloon_ground_immunity(fresh_battle, step_turn, hp_of):
    state, prng = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        [MonSpec("tyranitar", ["tackle", "tackle", "tackle", "tackle"], item="airballoon")],
        seed=11,
    )
    hp1_pre = hp_of(state, 1)
    step_turn(state, prng, 0, 0)
...
def test_weakness_policy_boosts_on_se(fresh_battle, step_turn, boost_of):
    state, prng = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        [MonSpec("tyranitar", ["tackle", "tackle", "tackle", "tackle"], item="weaknesspolicy")],
        seed=13,
    )
    step_turn(state, prng, 0, 0)
...
def test_rocky_helmet_chips_contact_attacker(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        ...
        [MonSpec("ferrothorn", ["tackle", "tackle", "tackle", "tackle"], item="rockyhelmet")],
        seed=14,
    )
    hp0_pre = hp_of(state, 0)
    max0 = max_hp_of(state, 0)
...
def test_rocky_helmet_skipped_for_non_contact(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("garchomp", ["earthquake", "tackle", "tackle", "tackle"])],
        [MonSpec("ferrothorn", ["tackle", "tackle", "tackle", "tackle"], item="rockyhelmet")],
        seed=15,
...
def test_loaded_dice_multihit(fresh_battle, step_turn, hp_of):
    state_a, prng_a = fresh_battle(
        [MonSpec("cinccino", ["bulletseed", "tackle", "tackle", "tackle"], item="loadeddice")],
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"])],
        seed=16,
    )
    state_b, prng_b = fresh_battle(
...
def test_leftovers_heals_each_turn(fresh_battle, step_turn, hp_of, max_hp_of):
    ...
    state_a, prng_a = fresh_battle(
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], item="leftovers")],
        [MonSpec("garchomp", ["tackle", "tackle", "tackle", "tackle"])],
        seed=17,
    )
    state_b, prng_b = fresh_battle(
...
def test_black_sludge_heals_poison_type(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("venusaur", ["splash", "splash", "splash", "splash"], item="blacksludge")],
...
def test_black_sludge_damages_non_poison(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], item="blacksludge")],
        [MonSpec("garchomp", ["tackle", "tackle", "tackle", "tackle"])],
        seed=19,
    )
    max0 = max_hp_of(state, 0)
...
def test_focus_sash_survives_ohko(fresh_battle, step_turn, hp_of):
    ...
    state, prng = fresh_battle(
        [MonSpec("glaceon", ["icebeam", "tackle", "tackle", "tackle"], item="choicespecs")],
        [MonSpec("garchomp", ["splash", "splash", "splash", "splash"], item="focussash")],
        seed=20,
    )
    step_turn(state, prng, 0, 0)
...
def test_sitrus_berry_triggers_below_half(fresh_battle, step_turn, hp_of, max_hp_of):
    state, prng = fresh_battle(
        [MonSpec("snorlax", ["tackle", "tackle", "tackle", "tackle"], item="sitrusberry")],
        [MonSpec("garchomp", ["tackle", "tackle", "tackle", "tackle"])],
        seed=21,
    )
    max0 = max_hp_of(state, 0)
