...
def apply_knock_off_from_move(
    ...
    has_sticky_hold = (target_ability == ABILITY_STICKY_HOLD) and (user_ab_ko not in _MB_SET_KO)
    should_remove = (target_hp > 0) and (target_item > 0) and (not is_unremovable) and (not has_sticky_hold)
...
def apply_trick_from_move(
    ...
    target_lock_off = OFF_FIELD + (F_CHOICE_LOCK_1 if user_is_side0 else F_CHOICE_LOCK_0)
...
def apply_rapid_spin_from_move(
    ...
    leech_seed_offset = OFF_FIELD + (F_LEECH_SEED_0 if user_side == 0 else F_LEECH_SEED_1)
    ...
    from pokepy.core.constants import (
        F_EXTENDED_VOLATILE_0 as _FEV0, F_EXTENDED_VOLATILE_1 as _FEV1,
...
def apply_defog_from_move(
    ...
    from pokepy.core.constants import (
        SCREEN_REFLECT_SHIFT, SCREEN_LIGHTSCREEN_SHIFT,
        SCREEN_AURORAVEIL_SHIFT, SCREEN_SAFEGUARD_SHIFT, SCREEN_MIST_SHIFT,
        SCREEN_MASK_2BIT, SCREEN_MASK_3BIT,
    )
    ...
    if has_mirror_armor_df:
        ...
        user_active = int(battle[OFF_META + (M_ACTIVE0 if user_side == 0 else M_ACTIVE1)])
...
def _set_screen_turns(screens: int, shift: int, turns: int, should_set: bool, mask: int) -> int:
...
def apply_screen_from_move(
    ...
    new_screens = _set_screen_turns(
        new_screens, SCREEN_REFLECT_SHIFT, screen_turns, hit and is_reflect, SCREEN_MASK_3BIT
    )
    new_screens = _set_screen_turns(
        new_screens, SCREEN_LIGHTSCREEN_SHIFT, screen_turns, hit and is_light_screen, SCREEN_MASK_3BIT
    )
    new_screens = _set_screen_turns(
        new_screens, SCREEN_AURORAVEIL_SHIFT, screen_turns, hit and aurora_veil_can_set, SCREEN_MASK_3BIT
    )
    new_screens = _set_screen_turns(
    ...
    new_screens = _set_screen_turns(
        new_screens, SCREEN_LUCKYCHANT_SHIFT, 3, hit and is_lucky_chant, SCREEN_MASK_2BIT
    )
